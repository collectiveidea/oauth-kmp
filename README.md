 ...
 * Call [`startSignIn`](https://github.com/collectiveidea/oauth-kmp/blob/main/oauth-core/src/commonMain/kotlin/com/collectiveidea/oauth/PKCEFlow.kt#L100C16-L100C27) 
   * On Android, construct [`AndroidWebAuthSession`](https://github.com/collectiveidea/oauth-kmp/blob/main/oauth-core/src/androidMain/kotlin/com/collectiveidea/oauth/AndroidWebAuthSession.kt) with your `ComponentActivity` (as a field or early in `onCreate`, before the activity is STARTED), and pass `PKCEFlow::continueSignInWithCallbackOrError` as its `completionHandler`. Providing that handler at construction (rather than per `startSignIn` call) is what lets an Auth Tab sign-in interrupted by an activity recreation (e.g. a rotation) still complete when its result is redelivered to the rebuilt flow. Because `AndroidWebAuthSession` must be rebuilt for each `Activity` while the `PKCEFlow` is typically an app-wide singleton, don't hand it to the singleton directly. Instead use the provided [`CurrentActivityWebAuthSession`](https://github.com/collectiveidea/oauth-kmp/blob/main/oauth-core/src/androidMain/kotlin/com/collectiveidea/oauth/CurrentActivityWebAuthSession.kt): give the `PKCEFlow` its `factory`, and call `bindTo(this)` from each `Activity`'s `onCreate`. It forwards to the current `Activity`'s `AndroidWebAuthSession` and releases it automatically on destroy (but not on a recreation, so an in-flight sign-in survives). When the browser supports an [Auth Tab](https://developer.chrome.com/docs/android/custom-tabs/guide-auth-tab) (Chrome 137+, i.e. Android 8+), the result is delivered directly and nothing else is needed. To keep working on older devices/browsers that fall back to a Custom Tab, also register the redirect scheme with an `<intent-filter>` and forward the redirect from `onNewIntent`:
    ```kotlin
    ...
    ```
   * On iOS, nothing else is needed. The [callback is automatically invoked](https://github.com/collectiveidea/oauth-kmp/blob/main/oauth-core/src/iosMain/kotlin/com/collectiveidea/oauth/IosWebAuthSession.kt#L26) by [`AuthenticationServices`](https://developer.apple.com/documentation/authenticationservices).
