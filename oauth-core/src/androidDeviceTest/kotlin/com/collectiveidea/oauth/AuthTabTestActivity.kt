 ...
 * Tests then drive the already-registered launcher via `scenario.onActivity { it.session... }`.
 *
 * The session's single completion handler records what it receives into [result] and releases
...
class AuthTabTestActivity : ComponentActivity() {
    val session = AndroidWebAuthSession(this) { callbackUrl, errorMessage ->
        result = callbackUrl to errorMessage
        resultLatch.countDown()
    }

    companion object {
        /** The last result the session reported. Reset it in tests before use. */
        @Volatile
        var result: Pair<String?, String?>? = null

        /** Released when the session reports a result. Reset it in tests before awaiting. */
        @Volatile
